...
export const generate = onRequest({ region: 'europe-west1', secrets: [GEMINI_API_KEY], timeoutSeconds: 120, memory: '512MiB' }, async (req, res) => {
    ...
    try {
        ...
        if (!subject || !course || !numQuestions) {
            res.status(400).json({ error: 'Missing subject/course/numQuestions' });
            return;
        }
        ...
        const fetchWithTimeout = async (url, init, ms = 90000) => {
        ...
        if (!listRes.ok) {
            ...
            res.status(400).json({ error: 'ListModels failed', details: txt });
            return;
        }
        ...
        const pick = (arr, token) => arr.find(m => String(m.name || '').includes(token));
        const chosen = pick(compatible, 'flash') || pick(compatible, 'pro') || compatible[0];
        if (!chosen?.name) {
            res.status(400).json({ error: 'No compatible model found', details: models.map(m => m?.name).filter(Boolean) });
            return;
        }
        ...
        let data = null;
        {
            ...
            for (let attempt = 1; attempt <= 2; attempt++) {
                ...
                try {
                    ...
                    errors.push(`${r.status}${j && j.error && j.error.message ? `: ${j.error.message}` : ''}`);
                }
                catch {
                    errors.push(String(r.status));
                }
            }
        }
        if (!data) {
            logger.error('Gemini error', errors);
            res.status(400).json({ error: 'Gemini error', details: errors });
            return;
        }
