...
export const generate = onRequest({ region: 'europe-west1' }, async (req, res) => {
  ...
  try {
    ...
    for (const base of candidates) {
      ...
      try { const j = await r.json(); errors.push(`${r.status}${j?.error?.message?`: ${j.error.message}`:''}`) } catch { errors.push(String(r.status)) }
    }
    if (!data) {
    ...
    try { parsed = JSON.parse(text) }
      ...
      if (!match) return res.status(400).json({ error: 'Invalid JSON from model' })
